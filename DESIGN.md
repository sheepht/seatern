...
- **Specific animations:**
  ...
  - 智慧推薦虛線：`stroke-dashoffset` 流動動畫（0.6s linear infinite）

## 智慧推薦虛線

Hover 賓客時顯示的推薦路徑線，指向系統建議的目標桌。

### 路徑計算
- **避障路由：** 使用 `pathfinding`（A* 網格尋路），20px 格解析度，桌子圓形 + 12px padding 為障礙
- **曲線平滑：** A* 路徑經 Catmull-Rom 樣條轉 cubic bezier，產生 SVG `<path>`
- **無障礙時：** 使用 quadratic bezier 弧線（偏移距離 15%），不畫直線
- **出桌機制：** 來源桌也是障礙，路徑先往桌外走再轉向目標（桌子視覺蓋住出發段）
...
| 線色 | `#B08D57`（accent） |
| 線寬 | 2.5px |
| 虛線 | `strokeDasharray="10 6"` |
| 箭頭 | 16px 三角形，顏色同線，方向跟隨曲線末端切線 |
| +N badge | 綠底白字（`#16A34A`），40×24px 圓角矩形，位於箭頭前 ~100px |
| 透明度 | 第 1 條 0.9，第 2 條 0.75，第 3 條 0.60 |
| 最多條數 | 3 條 |

### 圖層順序（SVG）
推薦虛線在桌子**下層**渲染，桌子自然蓋住穿過的部分。

### 已入座 vs 待排賓客
| | 已入座 | 待排區 |
|--|-------|-------|
| 起點 | 賓客座位圓邊緣 | 側欄 chip 位置（SVG overflow: visible） |
| badge 文字 | `+N`（滿意度 delta） | `N`（預測滿意度分數） |
| 推薦條件 | 賓客 delta > 0 且桌/全場 delta > 0 | 預測分數 > 55 |
| chip 壓線 | 桌子蓋住出發段 | `.guest-chip:hover { z-index: 10 }` |
...
| 2026-03-27 | 推薦虛線改避障曲線 | 用 pathfinding A* + Catmull-Rom 取代直線，避開中間桌子 |
| 2026-03-27 | 待排賓客支援推薦線 | 從側欄 chip 出發，SVG overflow: visible + z-index 處理圖層 |
