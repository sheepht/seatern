...
將 MVP + V1.1 功能拆分為 **8 個階段**（含前置階段 0），每個階段結束後系統皆可正常運行並驗證。
    ...
    → 2 (CRUD API)
      → 3 (Auth)
        → 4 (Guest List UI)
          ...
          → 6 (排位工作區：桌次+拖曳+滿意度+預覽)   ← 可與 5 並行
            → 7 (Social Graph)     ← 7/8 可並行
            → 8 (Export & Polish)  ← 7/8 可並行
```
...
## 階段 6：排位工作區（桌次管理 + 拖曳 + 滿意度 + 即時回饋）

**目標**：建立完整的排位工作區 — 桌次平面圖、拖曳排位、滿意度即時計算、移動預覽、避免同桌警告

**架構原則**：排位操作全部在前端 in-memory 執行，按「儲存」才寫回 DB。滿意度計算邏輯放在 `packages/shared`，前後端共用。

**涉及套件**：`packages/shared`、`packages/api`、`packages/web`

**具體任務**：

**Shared（`packages/shared`）：**
1. 建立 `src/satisfaction.ts` — 滿意度計算引擎：
...
2. 排位載入/儲存：
   - `GET /api/events/:eventId/seating` — 載入排位工作區所有資料（桌次、賓客、標籤、偏好、避免同桌）
   - `POST /api/events/:eventId/seating` — 儲存當前排位狀態（所有賓客指派、桌次位置、滿意度分數）
3. 避免同桌：
...
2. 排位工作區（`/events/:eventId/seating`）：
   - `SeatingWorkspace.tsx` — 進入時呼叫 GET seating 載入所有資料到 in-memory state
   - 畫布：自由拖曳桌次平面圖（圓桌 + positionX/Y）
   - 各桌顯示：桌名、容量、已指派賓客、平均滿意度色條
   - 未指派賓客面板
   - `GuestChip.tsx` — 可拖曳的賓客標籤（顯示姓名、category 顏色、滿意度色點）
3. 拖曳邏輯（全部 in-memory）：
   - 未指派 → 桌次：指派
   - 桌次 → 桌次：移動
   - 桌次 → 未指派列表：取消指派
   - 桌次已滿 → 拒絕放置 + 視覺回饋
   - 每次操作後即時重算滿意度（呼叫 shared 的計算函式）
4. 交換模式：選兩位賓客互換座位
5. 移動預覽：
   - 拖曳懸停時顯示分數變化預覽（before → after、淨效益）
   - 連帶影響的賓客分數變化
6. 避免同桌警告：
   - 放到同桌時顯示警告（原因 + 建議替代方案）
   - 可選「我知道了，仍要執行」
7. 桌次編輯：修改桌名、顏色、容量（in-memory）
8. 統計列：全場平均滿意度 + 四色分佈 + 已指派/未指派計數
9. `ScoreBreakdownPanel` — 點擊賓客查看分數組成明細
10. 復原功能（Undo/Redo）
11. 「儲存排位」按鈕 → 呼叫 POST seating 寫回 DB

**驗證方式**：
1. 建立 3 張桌次（容量 10、10、8）→ 確認桌次卡片出現
2. 進入排位工作區 → 確認畫布顯示空桌 + 未指派賓客面板
3. 拖曳賓客到桌 → 確認滿意度即時更新
4. 將 3-4 位同標籤賓客指派到同桌 → 確認滿意度 > 50（有群組加分）
5. 將賓客移離想同桌的人 → 確認分數即時下降
6. 3 位想同桌的人全在同桌 → 確認偏好分為 +25
7. 拖 11 人到容量 10 的桌 → 第 11 人被拒絕
8. 交換兩位賓客 → 確認雙方分數即時更新
9. 標記兩人「避免同桌」→ 放到同桌 → 確認警告出現
10. 點擊賓客 → 確認顯示分數明細（基礎 50 + 群組 +X + 偏好 +X + 需求 +5）
11. 點「復原」→ 確認回到上一步
12. 點「儲存排位」→ 重新整理頁面 → 確認排位仍然正確（已持久化到 DB）

**預期結果**：完整的排位工作區。所有操作即時反映滿意度變化（in-memory），儲存時才寫回 DB。支援交換、預覽、避免同桌警告、復原。

---

## 階段 7：社交圖譜與 Community Detection
...
**Shared（`packages/shared`）：**
1. `src/optimizer.ts` — 自動優化建議引擎（in-memory，前後端共用）：
   - `generateSuggestions()` — 掃描低分（<70）賓客，搜索可能的移動/交換，計算淨效益，過濾正效益 + 無違規項，回傳 top 5-10 建議
...
**Web 端：**
1. `SuggestionPanel.tsx` — 建議面板（in-memory 計算，不打 API）：
...
| `packages/shared/src/satisfaction.ts` | 滿意度計算引擎（前後端共用） |
